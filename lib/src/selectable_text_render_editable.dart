...
///Copy from [RenderEditable]
class SelectableTextRender extends RenderBox {
  ...
  SelectableTextSelectionChangedHandler onSelectionChanged;
  ...
  ///call when [_paintContents]
  PaintContentHandler onPaintContent;
  ...
  int _handleShift(bool rightArrow, bool leftArrow, bool shift, int newOffset) {
    ...
    if (shift) {
      if (_baseOffset < newOffset) {
        onSelectionChanged(
          TextSelection(baseOffset: _baseOffset, extentOffset: newOffset),
          this,
          SelectionChangedCause.keyboard,
        );
      } else {
        onSelectionChanged(
          TextSelection(baseOffset: newOffset, extentOffset: _baseOffset),
          this,
          SelectionChangedCause.keyboard,
        );
      }
  ...
  double _preferredHeight(double width) {
    if (maxLines != null) return preferredLineHeight * maxLines;
  ...
  Rect get _getCaretPrototype {
    switch (defaultTargetPlatform) {
      case TargetPlatform.iOS:
        return Rect.fromLTWH(0.0, -_kCaretHeightOffset + .5, cursorWidth,
            preferredLineHeight + 2);
      default:
  ...
  void _paintContents(PaintingContext context, Offset offset) {
    ...
    if (onPaintContent != null)
      onPaintContent(this._textPainter, context.canvas);
