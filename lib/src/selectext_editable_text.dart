...
import 'package:flutter_selectext/src/selectable_rich_text.dart';
import 'package:flutter_selectext/src/selectable_text_render_editable.dart';
import 'package:flutter_selectext/src/selectext_selection.dart';
...
class SelectableTextEditingController extends TextEditingController {
  SelectableTextEditingController({String text})
      : _textSpan = null,
        super(text: text);

  SelectableTextEditingController.rich(this._textSpan) : super(text: _textSpan.toPlainText());

  TextSpan _textSpan;

  TextSpan get textSpan => this._textSpan;

  set textSpan(TextSpan textSpan) {
    _textSpan = textSpan;
    super.text = _textSpan.toPlainText();
  }
}

///Copy from [EditableText]
class SelectableTextEditableText extends StatefulWidget {
  ...
  final SelectableTextEditingController controller;
  ...
  final SelectableTextSelectionControls selectionControls;
  ...
  /// location).
  final FlutterSelectionChangedCallback onSelectionChanged;
...
/// State for a [SelectableTextEditableTextState]. Copy from [EditableTextState]
class SelectableTextEditableTextState extends State<SelectableTextEditableText>
    with
        AutomaticKeepAliveClientMixin<SelectableTextEditableText>,
        WidgetsBindingObserver,
        TickerProviderStateMixin<SelectableTextEditableText>
    implements TextInputClient, SelectableTextSelectionDelegate {
  Timer _cursorTimer;
  ...
  SelectableTextSelectionOverlay _selectionOverlay;
  ...
  @override
  void didUpdateWidget(SelectableTextEditableText oldWidget) {
...
//      widget.focusNode.requestFocus();
      ...
      final Iterable<FocusNode> iterable = FocusScope.of(context).ancestors;
      List<FocusScopeNode> focusScopeNodeList = List();
  ...
  void _handleSelectionChanged(
      TextSelection selection, SelectableTextRender renderObject, SelectionChangedCause cause) {
    ...
    if (widget.selectionControls != null) {
      _selectionOverlay = SelectableTextSelectionOverlay(
        context: context,
        value: _value,
  ...
  @visibleForTesting
  SelectableTextSelectionOverlay get selectionOverlay => _selectionOverlay;
  ...
  SelectableTextRender get renderEditable => _editableKey.currentContext.findRenderObject();
  ...
  void bringIntoView(TextPosition position) {
    _scrollController
        .jumpTo(_getScrollOffsetForCaret(renderEditable.getLocalRectForCaret(position)));
  }
  ...
  VoidCallback _semanticsOnCopy(SelectableTextSelectionControls controls) {
  ...
  Widget build(BuildContext context) {
    widget.focusNode.attach(context).reparent();
        ...
        return CompositedTransformTarget(
          ...
          child: Semantics(
            ...
            child: SelectableRichText(
              ...
              onPaintContent: widget.onPaintContent,
            ),
          ),
        );
      },
  ...
  TextSpan buildTextSpan() {
    if (widget.controller.textSpan != null) {
      return widget.controller.textSpan;
    }

    String text = _value.text;
    if (widget.obscureText) {
      text = SelectableTextRender.obscuringCharacter * text.length;
